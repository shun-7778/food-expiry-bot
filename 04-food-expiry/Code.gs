 ...
 */
var COMMAND_BUTTONS = ['期限登録', '使用済', '破棄済', '買い物リスト追加', '買い物リスト削除']
  .map(function (word) { return { label: word, fill: word + ' ' }; });

// 接頭辞として認める語。ボタンが差し込む語のほかに、手打ちしそうな言い方も拾う。
// COMMAND_BUTTONS の語は必ずここに含めること（testConfig で検査している）
var COMMAND_WORDS = {
  '期限登録': 'register',
  '登録': 'register',
  '使用済': 'consume',
...
function testConfig() {
  ...
  // ボタンが差し込む語を COMMAND_WORDS に足し忘れると、
  // 押しても普通の文として扱われてしまう。目に見えにくいのでここで検査する
  COMMAND_BUTTONS.forEach(function (b) {
    var cmd = parseCommand_(b.fill + 'テスト');
    console.log('ボタン「' + b.label + '」: '
      + (cmd ? cmd.action + ' OK' : '*** COMMAND_WORDS に未登録 ***')
      + (b.label.length <= 20 ? '' : ' / *** ラベルが20文字を超過 ***'));
  });
}
